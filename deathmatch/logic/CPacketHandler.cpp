            ...
            bitStream.ReadCompressed(usDimension) && bitStream.ReadBit(bIsAttached))
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CClientGame::VEHICLE:
                {
                    ...
                    unsigned int ucModel = 0;
                    bitStream.Read(ucModel);

                    unsigned int usModel = ucModel;

                    if (!(usModel >= 400 && usModel <= 611 || usModel >= 18593 && usModel <= 19093))
                    {
                        // SString strLine2("Ошибка протокола 39 , usModel2: %d ", usModel2;
                        SString strLine("Ошибка протокола 39 , айди тачки %d", usModel);
