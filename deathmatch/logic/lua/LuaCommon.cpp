...
void lua_push_element_fmod_core(lua_State* luaVM, FMOD::System* pFmod)
{
    luaL_getmetatable(luaVM, "FMOD");            // Получаем метастаблицу FMOD
    if (lua_isnil(luaVM, -1))                    // Если он не существует, создайте его
    {
        luaL_newmetatable(luaVM, "FMOD");
        lua_pushstring(luaVM, "__index");
        lua_pushvalue(luaVM, -2);
        lua_settable(luaVM, -3);
    }

    // Создайте новые пользовательские данные с указателем FMOD::System в качестве данных
    FMOD::System** ppFmod = (FMOD::System**)lua_newuserdata(luaVM, sizeof(FMOD::System*));
    *ppFmod = pFmod;

    // Установите метатаблицу для userdata в качестве метатаблицы FMOD
    luaL_getmetatable(luaVM, "FMOD");
    lua_setmetatable(luaVM, -2);
}

//void lua_push_element_fmod_core(lua_State* luaVM, FMOD::System* pFmod)
//{
//    FMOD::System** userdata =
//        static_cast<FMOD::System**>(lua_newuserdata(luaVM, sizeof(FMOD::System*)));            // создаем пользовательский тип данных FMOD::System**
//    *userdata = pFmod;            // передаем указатель на объект FMOD::System*
//
//    luaL_newmetatable(luaVM, "FMOD");
//    lua_pushstring(luaVM, "__index");
//    lua_pushvalue(luaVM, -2);
//    lua_settable(luaVM, -3);
//
//    lua_setmetatable(luaVM, -2);            // связываем метатаблицу с пользовательским типом данных FMOD::System**
//}
...
void lua_pushuserdata(lua_State* luaVM, void* pData)
{
    ...
    //else if (FMOD::System* pFMOD = UserDataCast((FMOD::System*)pData, luaVM))            // fmod 
    //    return lua_pushelement(luaVM, pFMOD);
